...
1. 사이클 폴더 생성: `cycles/<YYYY-MM>/` (파일럿은 `<YYYY-MM>-pilot`, 임시 사이클은 `<YYYY-MM>-<슬러그>`).
...
4. 인벤토리 증분 갱신: **SGT 저장소에서** `git -C ~/project/sgt log --oneline <inventory의 마지막 갱신 커밋>..HEAD`를 훑어 제품 기능 변화(새 API·모델 교체·배포 프리셋 추가)가 있으면 `sgt-feature-inventory.md`에 반영하고 머리말의 기준 커밋을 갱신한다.
...
2. Agent tool로 `issue-writer` 실행: `decision_path=<cycle>/4-decision.md, plan_path=<cycle>/2-plan.md`. 산출: `issue-drafts/pending/*.md`.
